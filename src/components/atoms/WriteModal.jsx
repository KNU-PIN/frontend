...
export default function WriteModal(props) {
  ...
  const { title, contents, pw, type, latitude, longitude, images } = values;
    ...
    <Container disappear={!props.visible}>
      <Header>
        <PinTitle>
          ...
          <PinCancel onClick={handleCancel}>게시글 작성 취소 X</PinCancel>
        </PinTitle>
      </Header>
      <FormWrapper>
        <Category>
          ...
          {typeButtonInfo.map((e, i) => {
            return (
              <TypeButton name={e.name} key={i} value={e.value} onClick={handleInputChange} color={e.color} values={values}>
                {e.text}
              </TypeButton>
            );
          })}
        </Category>
        <FileInput name="images" value={values.images} onChange={handleChange} accept="image/jpg,image/jpeg,image/jpe,image/png"></FileInput>
        <Wrapper>
          <Type>게시글 작성 ✒️</Type>
          <br />
          <StyledInput name="title" value={values.title} onChange={handleInputChange} placeholder="제목을 입력하세요." />
          <br />
          <StyledTextarea name="contents" value={values.contents} onChange={handleInputChange} placeholder="내용을 입력하세요." />
        </Wrapper>
        <Wrapper>
          <Type>비번번호 🔒︎</Type>
          <StyledInput2 name="pw" value={values.pw} onChange={handleInputChange} />
        </Wrapper>
        <Wrapper>
          <br />
